...
class CatalogoProductos extends \Controllers\PublicController
{
    ...
    private function _load($busqueda="")
    {
        if(empty($busqueda))
        {
            ...
            $_total = \Dao\Client\Libros::getProductCount();
            $_data = \Dao\Client\Libros::getProductosforPage($this->Start, $this->ProductLimit);
        }
        else
        {
            $this->PageTitle = "Resultados de la Búsqueda: ".$this->UsuarioBusqueda;
            $_total =  \Dao\Client\Libros::searchProductosClienteCount($this->UsuarioBusqueda);
            $_data =  \Dao\Client\Libros::searchProductosCliente($this->UsuarioBusqueda, $this->Start, $this->ProductLimit);
        }
        ...
        {
            if(strlen($_data[$key]["LibroDescripcion"]) > $max_description_length)
            {
                $string = $value["LibroDescripcion"];
                $offset = ($max_description_length - 3) - strlen($string);
                $_data[$key]["LibroDescripcion"] = substr($string, 0, strrpos($string, ' ', $offset)) . '...';
            }

            $precioFinal = ($value["LibroPrecioVenta"]) + ($value["LibroPrecioVenta"] * 0.15); 
            $_data[$key]["LibroPrecioVenta"] = number_format($precioFinal, 2);
        }
