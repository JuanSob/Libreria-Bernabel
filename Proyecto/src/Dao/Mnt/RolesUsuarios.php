...
class Rolesusuarios extends \Dao\Table
{
    public static function getAll()
    {
        ...
        FROM rolesusuario ru INNER JOIN usuarios u ON ru.UsuarioId = u.UsuarioId WHERE u.UsuarioTipo != 'PBL';", array());
    }

    public static function getOne($UsuarioId, $RolId)
    {
        $sqlstr = "SELECT ru.*, u.UsuarioNombre, u.UsuarioEmail, u.UsuarioTipo 
        FROM rolesusuario ru INNER JOIN usuarios u ON ru.UsuarioId = u.UsuarioId 
    ...
    {
        $updsql = "UPDATE rolesusuario SET RolUsuarioEst=:RolUsuarioEst, RolUsuarioExp=:RolUsuarioExp WHERE Usuarioid=:UsuarioId AND RolId=:RolId;";
        return self::executeNonQuery(
            $updsql,
